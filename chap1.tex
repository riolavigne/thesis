...
%Then, there are Zero-Knowledge Proofs (ZKP), which, in terms of functionality, allow a user to prove they know something, while keeping the actual knowledge a secret. While this may seem like a contradiction, it is currently used in identity protocols where users can prove who they are, and is deployed in the real-world blockchain ZCash \cite{TODO}.
...
\begin{theorem*}[\THC~from LWE informal]
...
\begin{theorem*}[\THC~Impossibility]
...
\begin{itemize}
	...
	\begin{theorem*}[Reconstructing Predicates Informal]
	...
	\begin{theorem*}[Gap-Hamming PPH from CRHFs Informal]
	...
	\begin{theorem*}[Gap-Hamming PPH from Syndrome Decoding Informal]
...
This thesis uses hypotheses on the running times of problems that, while solvable in polynomial time, are variants of natural NP-hard problems, in which the size of the solution is a fixed constant. For instance, \kSum~is the variant of Subset Sum, where we are given $n$ numbers and we need to find exactly $k$ elements that sum to a given target, and \zkclique~is the variant of Zero-Clique, in which we are given a graph and we need to find exactly $k$ nodes that form a clique whose edge weights sum to zero.
...
Despite these difficulties, we achieved the following results, presented in Chapter \ref{chap:FGC}:
\begin{itemize}
...
%		Assuming that a fine-grained problem requires $\~\Omega(T(n))$ time to solve, and this problem is plantable, list-hard, and splittable, then there exists a fine-grained key exchange 
%	\end{theorem*}
...
%	\begin{theorem*}[Quadratic-Gap Key-Exchange Informal]
%		TODO
%	\end{theorem*}
\end{itemize}
